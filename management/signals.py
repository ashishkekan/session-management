...
def log_session_save(sender, instance, created, **kwargs):
    ...
    log_activity(
        instance.conducted_by, action, description, details={"session_id": instance.id}
    )
...
def log_session_delete(sender, instance, **kwargs):
    log_activity(
        ...
        "DELETE",
        f"Deleted session: {instance.topic}",
        details={"session_id": instance.id},
    )
...
def log_department_save(sender, instance, created, **kwargs):
    ...
    log_activity(
        None,
        action,
        f"{'Created' if created else 'Updated'} department: {instance.name}",
        details={"department_id": instance.id},
    )
...
def log_department_delete(sender, instance, **kwargs):
    log_activity(
        None,
        "DELETE",
        f"Deleted department: {instance.name}",
        details={"department_id": instance.id},
    )
